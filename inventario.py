...
def cargar_csv():
    ...
    inventario_cargado = {}  # guarda los productos válidos del archivo
    errores = 0              # cuenta las filas que tienen algún problema

    try:
        with open(ruta, "r", encoding="utf-8") as f:
            reader = csv.reader(f)  # lee el archivo fila por fila como listas

            # Lee la primera fila y verifica que sea el encabezado correcto
            encabezado = next(reader, None)
            if encabezado != ["nombre", "precio", "cantidad"]:
                print("Error: el encabezado debe ser: nombre,precio,cantidad")
                print(f"Encabezado encontrado: {encabezado}")
                volver_menu()
                return

            # Recorre cada fila de datos (desde la segunda en adelante)
            for fila in reader:
                ...
                # Desempaca los 3 valores de la fila en variables separadas
                nombre, precio, cantidad = fila

                try:
                    precio = float(precio)    # convierte de texto a decimal
                    cantidad = int(cantidad)  # convierte de texto a entero
                    ...
                    # Guarda el producto como diccionario incluyendo costo_total
                    inventario_cargado[nombre] = {
                        "precio": precio,
                        "cantidad": cantidad,
                        "costo_total": precio * cantidad  # calculado al cargar
                    }

                except ValueError:
                    # precio o cantidad no eran números válidos
                    errores += 1
                    continue
        ...
        # El archivo no está guardado en UTF-8
        print("Error: problema de codificación, usa UTF-8.")
        volver_menu()
        return
    except OSError as e:
    ...
    # Si no quedó ningún producto válido, no hay nada que cargar
    ...
    if opcion == "S":
        # Reemplaza todo el inventario con lo que viene del archivo
        inventario = inventario_cargado
        accion = "reemplazo"

    elif opcion == "N":
        # Fusiona: agrega productos nuevos y actualiza los que ya existen
        # Política: suma cantidades, actualiza precio al nuevo
        for nombre, datos in inventario_cargado.items():
            if nombre in inventario:
                # El producto ya existe → suma cantidad y actualiza precio
                inventario[nombre]["cantidad"] += datos["cantidad"]
                inventario[nombre]["precio"] = datos["precio"]
                # Recalcula el costo total con los nuevos valores
                inventario[nombre]["costo_total"] = (
                    inventario[nombre]["precio"] * inventario[nombre]["cantidad"]
                )
            else:
                # El producto es nuevo → se agrega directamente
                inventario[nombre] = datos

        accion = "fusión"

    else:
        # El usuario no escribió S ni N → no se hace nada
